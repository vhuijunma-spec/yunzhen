...
def api_script_generate():
    ...
    scene_count = int(data.get("scene_count") or 4)
    if not idea: return jsonify({"code": 400, "message": "请输入故事创意"}), 400
    if scene_count < 1: scene_count = 1
    if scene_count > 12: scene_count = 12
    ...
    ch_row = conn.execute("""
        ...
        WHERE ch.status = 'active' AND cmp.model_name IN ('DeepSeek-V3', 'GLM-5.0', 'Kimi-K2.5', 'qwen-plus')
        ORDER BY ch.id LIMIT 1
    """).fetchone()
    if not ch_row:
        ch_row = conn.execute("SELECT id, name, base_url, api_key FROM channels WHERE status='active' ORDER BY id LIMIT 1").fetchone()
    conn.close()

    if not ch_row:
        return jsonify({"code": 500, "message": "没有可用的AI模型"}), 500

    api_key = ch_row["api_key"] or CTYUN_API_KEY or GOGO_API_KEY
    model = "DeepSeek-V3"

    prompt = f"""你是一个专业的视频分镜师。根据以下故事创意，生成{scene_count}个分镜镜头。

故事创意：{idea}

请用JSON格式输出，每个分镜包含：
- scene: 镜头编号
- description: 镜头描述（一句话）
- prompt: 用于AI视频生成的英文提示词（详细描述画面、光线、风格、构图）
- duration: 建议时长（5-10秒）
- camera: 运镜方式（如：固定、推近、拉远、跟拍、俯拍）

只输出JSON数组，不要其他内容。"""

    try:
        resp = requests.post(
            f"{ch_row['base_url']}/chat/completions",
            headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
            json={"model": model, "messages": [{"role": "user", "content": prompt}], "max_tokens": 3000, "temperature": 0.9},
            timeout=60
        )
        ...
        if json_match:
            ...
            return jsonify({"code": 500, "message": "AI返回格式异常，请重试", "raw": content[:500]}), 500
        return jsonify({"code": 0, "scenes": scenes, "model": model})
    except Exception as e:
        logger.error("脚本生成失败: %s", e)
        return jsonify({"code": 502, "message": f"AI服务调用失败: {e}"}), 502
...
def api_generate_batch():
    ...
    if not scenes or not isinstance(scenes, list):
        return jsonify({"code": 400, "message": "请提供分镜列表"}), 400

    results = []
    for s in scenes:
        prompt_text = (s.get("prompt") or s.get("description") or "").strip()
        if not prompt_text:
            continue
        # 调用单任务生成逻辑
        model = data.get("model") or "doubao-seedance-2-0-480p"
        ...
        if uid:
            conn = _get_db()
            parent = conn.execute("SELECT parent_id FROM users WHERE id=?", (uid,)).fetchone()
            bill_uid = parent["parent_id"] if parent and parent["parent_id"] else uid
            bal = conn.execute("SELECT points FROM user_balance WHERE user_id=?", (bill_uid,)).fetchone()
            points = bal["points"] if bal else 0
            cost = duration * 1  # 1积分/秒
            if points < cost:
                conn.close()
                results.append({"scene": s.get("scene", "?"), "status": "no_points", "message": f"积分不足(需{cost})"})
                continue
            # 扣积分
            conn.execute("UPDATE user_balance SET points=points-? WHERE user_id=?", (cost, bill_uid))
            conn.commit()
            conn.close()
        ...
        try:
            conn2 = _get_db()
            model_ch = conn2.execute("""
                SELECT ch.id, ch.name, ch.base_url, ch.api_key FROM channels ch
                JOIN channel_model_pricing cmp ON cmp.channel_id = ch.id
                WHERE cmp.model_name=? AND ch.status='active' LIMIT 1
            """, (model,)).fetchone()
            if not model_ch:
                model_ch = conn2.execute("SELECT id, name, base_url, api_key FROM channels WHERE status='active' ORDER BY id LIMIT 1").fetchone()
            conn2.close()

            if model_ch:
                api_key = model_ch["api_key"] or GOGO_API_KEY or CTYUN_API_KEY
                is_ty = model_ch["name"] == "天翼云"
                if is_ty:
                    video_params = {"model": model, "prompt": prompt_text, "metadata": {"ratio": size, "duration": duration, "generate_audio": True, "watermark": False}}
                    video_path = "/video/generations"
                else:
                    video_params = {"model": model, "prompt": prompt_text, "seconds": str(duration), "size": size}
                    video_path = "/videos"
                resp = requests.post(f"{model_ch['base_url']}{video_path}", headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, json=video_params, timeout=60)
                submit_result = resp.json()
                remote_id = submit_result.get("id") or submit_result.get("task_id", "")

                task_id = str(uuid.uuid4())
                _task_store[task_id] = {
                    "model": model, "text": prompt_text[:200], "status": "queued",
                    "remote_task_id": remote_id, "channel_name": model_ch["name"],
                    "query_url_path": "/videos/" if not is_ty else "/video/generations/",
                    "video_url": "", "content": "", "owner_id": uid,
                    "username": session.get("username", ""), "created_at": time.time(),
                }
                _save_task_to_db(task_id, _task_store[task_id])
                results.append({"scene": s.get("scene", "?"), "task_id": task_id, "status": "queued", "message": f"已提交"})
            else:
                results.append({"scene": s.get("scene", "?"), "status": "error", "message": "无可用渠道"})
        except Exception as e:
            results.append({"scene": s.get("scene", "?"), "status": "error", "message": str(e)[:100]})
